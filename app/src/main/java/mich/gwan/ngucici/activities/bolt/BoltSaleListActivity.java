...
import mich.gwan.ngucici.adapters.bolt.BoltSaleRecyclerAdapter;
import mich.gwan.ngucici.adapters.pdf.PDFBolt;
import mich.gwan.ngucici.databinding.ActivityBoltssalesListBinding;
...
public class BoltSaleListActivity extends AppCompatActivity implements PDFBolt.OnDocumentClose {
    ...
    private String path(){
        ...
        {
            path = Environment.getExternalStorageDirectory() + "/Sarensa/Ngucici/" + LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyyMMdd_HHmmss"))+"_BoltSales.pdf";
        }
        return path;
    }
    ...
    public void generateEXCEL(){
        ...
        for (int row = 0; row < list.size(); row++) {
            ...
            BoltSale sales = list.get(row);
            xssfRow.createCell(0).setCellValue(sales.getBoltSaleDate());
            xssfRow.createCell(1).setCellValue(sales.getBoltSaleTime());
            xssfRow.createCell(2).setCellValue(sales.getBoltSalecategory());
            xssfRow.createCell(3).setCellValue(Double.parseDouble(String.valueOf(Double.valueOf(sales.getBoltSaleQnty()))));
            xssfRow.createCell(4).setCellValue(Double.parseDouble(String.valueOf(Double.valueOf(sales.getBoltSaleUprice()))));
            xssfRow.createCell(5).setCellValue(Double.parseDouble(String.valueOf(Double.valueOf(sales.getBoltSaleTotal()))));
            xssfRow.createCell(6).setCellValue(Double.parseDouble(String.valueOf(Double.valueOf(sales.getBoltSaleProfit()))));
        }
        ...
        String filename = m.format(t) + "_Ngucici_bolt_sale_data.xls";
        ...
        AlertDialog.Builder builder =  new AlertDialog.Builder(BoltSaleListActivity.this);
