...
import mich.gwan.ngucici.adapters.spanner.SpannerRecyclerAdapter;
import mich.gwan.ngucici.databinding.ActivitySpannerListBinding;
...
public class SpannerListActivity extends AppCompatActivity {
    private RecyclerView recyclerView;
    private final AppCompatActivity activity = SpannerListActivity.this;
    private List<Spanner> list;
    private SpannerRecyclerAdapter recyclerAdapter;
    private DatabaseHelper databaseHelper;
    private ActivitySpannerListBinding binding;

    @Override
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);

        binding = ActivitySpannerListBinding.inflate(getLayoutInflater());
        ...
        binding.iconBack.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                SpannerListActivity.this.finish();
            }
        });
    ...
    private void initViews(){
        recyclerView = binding.recyclerViewSpanner;
    }

    private void initObjects(){
        list = new ArrayList<>();
        recyclerAdapter = new SpannerRecyclerAdapter(list);
    ...
    private void deleteItem(int position) {
        ...
        databaseHelper.deleteSpanner(list.get(position));
    ...
    private void updatePrice(int note,int sp, int position) {
        Spanner n = list.get(position);
        // updating note text
        n.setSpanner_bp(note);
        n.setSpanner_sp(sp);

        // updating note in db
        databaseHelper.updateSpanner(n);
        // refreshing the list
        list.set(position, n);
     ...
     */
    private void showNoteDialog(final boolean shouldUpdate, final Spanner note, final int position) {
        ...
        if (shouldUpdate && note != null) {
            inputNote.setText(String.valueOf(note.getSpanner_bp()));
            sp.setText(String.valueOf(note.getSpanner_sp()));
            dialogItem.setText(note.getSpanner_category());
        }
